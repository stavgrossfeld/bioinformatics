...
def main(filename):
    ...
    for line in tqdm(sys.stdin, total=number_of_lines):
        ...
        if umi not in umi_dict:
            ...
            #umi_dict[umi] = {cb : [chr_location]}

            umi_dict[umi] = {"cb_code": [cb_chr], "ct": 1}
            # umi_dict_ct[umi] = 1
        else:
            # umi_dict[umi].append({cb_chr})
            if cb_chr in umi_dict[umi]["cb_code"]:
                pass
            else:
                umi_dict[umi]["cb_code"].append(cb_chr)
                umi_dict[umi]["ct"] += 1

            # umi_dict_ct[umi] += 1

    # print(umi_dict_ct)    # return umi_dict
        # print(umi_dict_ct)

    # umi_df = pd.DataFrame(pd.Series(umi_dict)).reset_index()
    #
    # print(umi_dict)
    # exit
    # print(umi_df.head())

    # umi_df.columns = ["umi", "ct"]
    ...
    jumped_index = umi_df[umi_df.ct > 1]
    print(jumped_index)
